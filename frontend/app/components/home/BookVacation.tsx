...
export default function BookVacation() {
  ...
  const onSubmit = async (data: formDataType) => {
    if (isCooldown) {
      toast.error("Form already submitted! Please wait 5 seconds.", { position: "top-right" });
      return;
    }
    ...
    try {
      ...
      toast.error(`${(error as Error).message} Try again!`, { position: "top-right" });
    }
  };

  const inputClass = "outline-none bg-slate-300 placeholder:text-black-500 text-md px-4 rounded-lg h-12 w-full";
    ...
    <div className="h-full w-full bg-gray-200 shadow-2xl mb-8">
      ...
      <div className="container flex justify-center h-full z-50 -translate-y-28 max-sm:-translate-y-20">
        <div className="w-full h-full p-6 shadow-xl rounded-3xl bg-gray-100">
          <form onSubmit={handleSubmit(onSubmit)} className="w-full rounded-2xl bg-slate-200 p-6 grid grid-cols-4 gap-x-6 gap-y-2">
            ...
            <div className="flex flex-col gap-x-6 gap-y-2 items-center col-span-full sm:col-span-2 sm:flex-row">
              <div className="w-full">
                <label htmlFor="travel-from" className={labelClass}>From</label>
                <div>
                  <input
                    type="text"
                    id="travel-from"
                    placeholder="Your origin location"
                    className={`${inputClass} ${errors.from ? errorInputClass : ""}`}
              ...
              <GoArrowSwitch className={["text-3xl max-md:text-5xl max-sm:text-3xl hidden md:inline w-28 mt-4", Colors.textPrimary].join(" ")} />

              {/* To Field */}
              <div className="w-full">
                <label htmlFor="travel-to" className={labelClass}>To</label>
                <div>
                  <input
                    type="text"
                    id="travel-to"
                    placeholder="Destination"
                    className={`${inputClass} ${errors.to ? errorInputClass : ""}`}
            ...
            <div className="flex flex-col gap-x-6 gap-y-2 items-center col-span-full sm:col-span-2 sm:flex-row">
              {/* Departing Date */}
              <div className="w-full">
                <label htmlFor="departing" className={labelClass}>Departing</label>
                <div>
                  <input
                    type="date"
                    id="departing"
                    className={`${inputClass} ${errors.departing ? errorInputClass : ""}`}
                    {...register("departing", { required: "Departing date is required" })}
                    defaultValue={new Date().toISOString().split("T")[0]}
                  />
                  {errors.departing && <p className="text-red-500">{errors.departing.message}</p>}
                </div>
              </div>

              <FaArrowsLeftRightToLine className={["text-3xl max-md:text-5xl max-sm:text-3xl hidden md:inline w-28 mt-8", Colors.textPrimary].join(" ")} />

              {/* Returning Date */}
              <div className="w-full">
                <label htmlFor="returning" className={labelClass}>Returning</label>
                <div>
                  <input
                    type="date"
                    id="returning"
                    className={`${inputClass} ${errors.returning ? errorInputClass : ""}`}
                    {...register("returning", { required: "Returning date is required" })}
                    defaultValue={new Date().toISOString().split("T")[0]}
                  />
                  {errors.returning && <p className="text-red-500">{errors.returning.message}</p>}
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center col-span-2 sm:col-span-1">
              <label htmlFor="passengers_count" className={labelClass}>Total Passengers</label>
              <div>
                <input
                  type="number"
                  min={1}
                  className={`${inputClass} text-center ${errors.passengers ? errorInputClass : ""}`}
                  {...register("passengers", {
                    required: "Number of passengers is required",
                    min: { value: 1, message: "At least one passenger required" },
                    max: { value: 100, message: "Maximum 100 passengers allowed" },
                  })}
                  defaultValue={1}
                />
