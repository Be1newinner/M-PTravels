...
const UserDetailsForm = ({ formData, setFormData, formAction }: UserDetailsFormProps) => {
  ...
  return (
    <form onSubmit={formAction}>
      <div className="p-4 py-10 w-full bg-white shadow-lg rounded-lg">
        ...
        <div className="flex flex-wrap max-sm:flex-wrap mt-4">
          ...
          {[
            ...
            { name: "no_of_people", placeholder: "No. of People", type: "number" },
            { name: "pickup_address", placeholder: "Your Pickup Address!", type: "text" },
          ].map((input) => (
            <div key={input.name} className="h-12 w-1/2 max-sm:w-full pr-4 max-sm:pr-0 my-4 nth-last-[2]:w-full">
              <input
                type={input.type}
                name={input.name}
      ...
      <div className="mt-8 p-4 py-8 w-full bg-white shadow-lg rounded-lg">
        ...
        <div className="flex flex-wrap max-sm:flex-wrap mt-4">
          {[
            ...
            <div key={dateInput.name} className="h-12 w-1/2 max-sm:w-full pr-4 max-sm:pr-0 my-4">
      ...
      {/** Submit Button */}
      <button type="submit" className="mt-8 mb-2 h-12 w-full bg-blue-500 text-white rounded-lg">
        Book Trip!
      </button>
    ...
    <div>
      <div className="w-full flex max-xl:flex-wrap gap-4 max-sm:px-2 max-xl:px-10 bg-gray-100">
        <div className="pl-16 max-xl:pl-0 pr-4 max-xl:pr-0  pt-4 h-full w-8/12 max-xl:w-full">
          <UserDetailsForm formData={formData} setFormData={setFormData} formAction={handleFormSubmit} />
        </div>
